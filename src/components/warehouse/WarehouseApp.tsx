...
export default function WarehouseApp() {
  ...
  async function importMigration(file: File) {
    ...
    try {
      ...
      const inventoryCount = Object.keys(result.state.inventory.counts).length;
      const receiptCount = Object.keys(result.state.receipt.items).length;
      if (!inventoryCount && !receiptCount && !result.state.receipt.supplier && !result.state.receipt.deliveryNote) {
        throw new Error("In dieser Datei wurde kein alter Inventur- oder Wareneingangs-Entwurf gefunden. Bitte den Export im ursprünglichen Browser der alten Lager-App erstellen.");
      }
      setView(inventoryCount ? "inventory" : "receipt");
      setFeedback({ kind: "success", message: inventoryCount + " Inventurpositionen und " + receiptCount + " Wareneingangspositionen wurden übernommen." });
  ...
  return <div className="mx-auto max-w-[1500px]">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-wide text-[var(--ph-gold)]">Palmenheld Hub</p><h1 className="mt-1 text-3xl text-[var(--ph-green-dark)]">Lager-App</h1><p className="mt-2 max-w-3xl text-slate-500">Mobile Lagerverwaltung mit Live-Daten aus Weclapp und gemeinsamen, geräteübergreifenden Entwürfen.</p></div><div className="flex flex-wrap items-center gap-2">{badge}<a href="https://app.palmenheld.de/migration-export.html" target="_blank" rel="noreferrer" className="rounded-xl border border-blue-500 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-blue-900">Alten Entwurf exportieren</a><button type="button" onClick={() => migrationInput.current?.click()} disabled={busy} className="rounded-xl border border-blue-500 bg-blue-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">Sicherungsdatei importieren</button><button type="button" onClick={() => load(true)} disabled={loading} className="rounded-xl border px-4 py-2.5 text-sm font-semibold">Neu laden</button></div></header>
    <input ref={migrationInput} type="file" accept="application/json,.json" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importMigration(file); event.currentTarget.value = ""; }}/>
