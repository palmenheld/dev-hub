    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "Produktive Veröffentlichung ist erst mit einem freigegebenen Kleinanzeigen-Partneradapter möglich.",
      },
      { status: 503 }
    );
  }

  const listing = await updateKleinanzeigenListingStatus(id, "active", {
    externalId: `mock-${id}`,
    lastError: undefined,
  });
